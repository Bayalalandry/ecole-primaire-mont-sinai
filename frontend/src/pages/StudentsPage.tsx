        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 relative z-30">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-white drop-shadow-lg">Gestion des Élèves</h1>
                <p className="text-xs sm:text-sm text-blue-100 drop-shadow">Liste complète des élèves</p>
              </div>
            </div>
          ...
          {user?.role === 'teacher' ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
                <div className="bg-white rounded-xl shadow-md p-4 sm:p-6 border-l-6 border-blue-500">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-2xl sm:text-4xl font-bold text-blue-600">{filteredStudents.length}</p>
                    </div>
                ...
                <div className="bg-white rounded-xl shadow-md p-4 sm:p-6 border-l-6 border-emerald-500">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-1">Actifs</h3>
                      <p className="text-2xl sm:text-4xl font-bold text-emerald-600">{filteredStudents.filter(s => s.status === 'active').length}</p>
                    </div>
                    <div className="p-2 sm:p-3 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-full flex-shrink-0">
                      <CheckCircle className="w-5 h-5 sm:w-8 sm:h-8 text-white" />
                    </div>
                  </div>
                </div>
                <div className="bg-white rounded-xl shadow-md p-4 sm:p-6 border-l-6 border-amber-500">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-1">Redoublants</h3>
                      <p className="text-2xl sm:text-4xl font-bold text-amber-600">{filteredStudents.filter(s => s.status === 'repeating').length}</p>
                    </div>
                    <div className="p-2 sm:p-3 bg-gradient-to-br from-amber-400 to-amber-600 rounded-full flex-shrink-0">
                      <Clock className="w-5 h-5 sm:w-8 sm:h-8 text-white" />
                    </div>
                  </div>
                </div>
                <div className="bg-white rounded-xl shadow-md p-4 sm:p-6 border-l-6 border-gray-500">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-1">Archivés</h3>
                      <p className="text-2xl sm:text-4xl font-bold text-gray-600">{filteredStudents.filter(s => s.status === 'archived').length}</p>
                    </div>
                    <div className="p-2 sm:p-3 bg-gradient-to-br from-gray-400 to-gray-600 rounded-full flex-shrink-0">
                      <Archive className="w-5 h-5 sm:w-8 sm:h-8 text-white" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 sm:space-y-6">
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Élèves par classe</h2>
                {Object.keys(studentsByClass).length === 0 ? (
                  <div className="bg-white rounded-xl shadow-card overflow-hidden">
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                          <thead className="bg-gray-50">
                            <tr>
                              ...
                              <th className="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Prénom</th>
                              <th className="hidden sm:table-cell px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date de naissance</th>
                              <th className="hidden sm:table-cell px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Genre</th>
                              <th className="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Statut</th>
                              <th className="hidden md:table-cell px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Parent</th>
                              <th className="hidden md:table-cell px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Téléphone</th>
                          ...
                          <tbody className="bg-white divide-y divide-gray-200">
                            {(classStudents as any[]).map((student) => (
                              <tr key={student.id}>
                                <td className="px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900">{student.matricule || 'N/A'}</td>
                                <td className="px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900">{student.last_name}</td>
                                <td className="px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900">{student.first_name}</td>
                                <td className="hidden sm:table-cell px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900">{student.date_of_birth || 'N/A'}</td>
                                <td className="hidden sm:table-cell px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900">{student.gender === 'M' ? 'M' : 'F'}</td>
          ...
          <div className="modal-content p-6 max-w-2xl w-full mx-4 border-2 border-blue-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
              ...
              <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 sm:space-x-3 pt-4">
                ...
                <button
                  type="submit"
                  className="w-full sm:w-auto px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:from-blue-700 hover:to-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all shadow-sm hover:shadow-md flex items-center justify-center gap-2 text-sm"
                >
        ...
        <div className='modal-overlay'>
          <div className='modal-content p-4 sm:p-6 max-w-md w-full mx-2 sm:mx-4 border-2 border-blue-200'>
              ...
              {(user?.role === 'teacher' ? getOrderedClassNames(assignedClasses) : classes.map(c => c.name)).map((className) => (
                <label key={className} className='flex items-center space-x-3 cursor-pointer'>
                  <input
                    ...
                    className='w-4 h-4 text-blue-600 rounded focus:ring-blue-500'
                  />
                  <span className='text-gray-700 text-sm sm:text-base'>{className}</span>
                </label>
              ))}
            </div>
            <div className='flex flex-col-reverse sm:flex-row justify-end gap-3 sm:space-x-3'>
              ...
              <button
                onClick={exportToPDF}
                className='w-full sm:w-auto px-4 py-2 bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-lg hover:from-red-600 hover:to-pink-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-all shadow-sm hover:shadow-md flex items-center justify-center gap-2 text-sm'
              >
                <FileText className="w-4 h-4" />
                PDF
              </button>
